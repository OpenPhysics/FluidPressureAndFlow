...
This sim was scaffolded by `npm run scaffold-screens` into `src/under-pressure/`, `src/flow/`, `src/water-tower/` (fleet naming:
...
1. Copies the `under-pressure/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `under-pressure/` folder
...
```
main.ts
  └─ UnderPressureScreen            (Screen<UnderPressureModel, UnderPressureScreenView>)
       ├─ UnderPressureModel         owns all state
       └─ UnderPressureScreenView    owns all visuals
```
...
template's `UnderPressureScreen.ts`. Screen icons live in one shared module under
...
```
src/common/FluidPressureAndFlowScreenIcons.ts
```
